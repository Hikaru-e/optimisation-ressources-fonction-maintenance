...
tools_availability = {tool: st.number_input(f"Disponibilité de {tool}", min_value=1, key=f"tool_availability_{i}") for i, tool in enumerate(tools_input)}

# Tâches et dépendances
st.header("Dépendances des Tâches")
dependencies = {}
for task in tasks_input:
    dependencies[task] = st.multiselect(f"Prérequis pour {task}", tasks_input, key=f"dependencies_{task}")
...
if st.button("Optimiser"):
    ...
    prob += lpSum(x[i, j] for i in tasks_input for j in resources_input), "Nombre_de_taches_completees"
